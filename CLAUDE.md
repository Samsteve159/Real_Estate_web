...
- **GoDaddy hosting (plan CORRECTED 2026-07-25 → VPS, not shared).** Production moves to a **Manifest-owned GoDaddy** account. Research (2026-07-25) confirmed GoDaddy **shared/cPanel** plans (Deluxe etc.) **cannot run the Node `api/`** (no terminal / long-running process) — so the earlier "Deluxe + verify Node" plan is dropped. Buy a **GoDaddy VPS: 1 vCPU / 2 GB RAM / 40 GB NVMe, Linux, Self-Managed, no cPanel add-on** (AUD ~$13.95/mo intro, renews ~$22.95; full root/SSH). No domain purchase (manifestre.com.au already owned) and no paid SSL (free Let's Encrypt). Claude sets it up via SSH: static site + Node `api/` (PM2/systemd) + nginx reverse-proxy `/api` + Let's Encrypt SSL. **Cutover:** deploy to a staging subdomain first → Akshay reviews → single DNS flip → old WordPress kept as rollback. Plan: Akshay buys under Manifest → shares cPanel/SSH → Claude inspects the environment, confirms Node support (or upgrade path), then deploys site + API + domain + SSL so Akshay gets a live working site. Spend is then attributed per-project in the Console **Cost dashboard** (filter by workspace) and the **Admin API cost report** (grouped by workspace; needs a separate `sk-ant-admin…` admin key). **Credit balance + auto-reload are org-level, not per-workspace** — attribution comes from the cost report, not earmarked credit. Org auto-top-up is **capped at $50** (Akshay pre-paid AUD 20 for the chatbot + instant-valuation tools).
